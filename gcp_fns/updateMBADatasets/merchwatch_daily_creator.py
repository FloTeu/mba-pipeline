...
from mwfunctions.pydantic.firestore.utils import get_default_category_name, get_bsr_top_category_names_list, get_bsr_category
...
def make_trend_column(marketplace, df_shirts, months_privileged=6):
    ...
    x = df_shirts[["time_since_upload"]].fillna(df_shirts["time_since_upload"].max()).values # prevent bug with pd.NA which does not work with np.nanmax
...
def update_bq_shirt_tables(marketplace, chunk_size=500, limit=None, filter=None, dev=False, project_id="mba-pipeline", num_threads=4):
    ...
    df_shirts_with_more_info["update_last"] = df_shirts_with_more_info["update_last"].astype(str)
    df_shirts_with_more_info["takedown_date"] = df_shirts_with_more_info["takedown_date"].astype(str)
